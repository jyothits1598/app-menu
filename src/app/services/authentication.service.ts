...
export class AuthenticationService {
  ...
  login({ email, password }, returnUrl) {
    if (email && password) {
      ...
      this.http.post(API_URL_LINK + 'signin-partner', loginDetails).subscribe(
        (tokenResponse: TokenResponse) => {
          if (tokenResponse && tokenResponse.access_token && tokenResponse.user_details) {
            ...
            localStorage.setItem('loggedUser', JSON.stringify(tokenResponse));
            this.isLoginSubject.next(true);
            this.userObjectSubject.next(tokenResponse);
            const userdata = tokenResponse.user_details;
